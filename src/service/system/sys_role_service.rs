...
impl SysRoleService {
    ...
    pub async fn query_sys_role_list(rb: &RBatis, item: QueryRoleListReq) -> ServiceResult<Page<RoleResp>> {
    ...
    pub async fn query_allocated_list(rb: &RBatis, item: AllocatedListReq) -> ServiceResult<Page<UserResp>> {
        ...
        ok_result_data(Page { total, items: list })
    }

    pub async fn query_unallocated_list(rb: &RBatis, item: UnallocatedListReq) -> ServiceResult<Page<UserResp>> {
        let page_no = item.page_no;
        let page_size = item.page_size;
        let role_id = item.role_id;
        let mobile = item.mobile.as_deref().unwrap_or_default();
