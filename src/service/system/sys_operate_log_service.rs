...
impl SysOperateLogService {
    pub async fn delete_sys_operate_log(rb: &RBatis, item: DeleteOperateLogReq) -> AppResult<Json<BaseResponse<String>>> {
        OperateLog::delete_by_map(rb, value! {"id": &item.ids}).await.map(|_| ok_result())?
    }

    pub async fn clean_sys_operate_log(rb: &RBatis) -> AppResult<Json<BaseResponse<String>>> {
        sys_operate_log_dao::clean_operate_log(rb).await.map(|_| ok_result())?
    }

    pub async fn query_sys_operate_log_detail(rb: &RBatis, item: QueryOperateLogDetailReq) -> AppResult<Json<BaseResponse<OperateLogResp>>> {
    ...
    pub async fn query_sys_operate_log_list(rb: &RBatis, item: QueryOperateLogListReq) -> AppResult<Json<PageResponse<Vec<OperateLogResp>>>> {
