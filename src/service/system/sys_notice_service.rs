...
use crate::common::result::{ok, ok_result, ok_result_data, ok_result_page, BaseResponse, EmptyResponse, PageResponse};
...
impl SysNoticeService {
    pub async fn add_sys_notice(rb: &RBatis, mut item: NoticeReq) -> AppResult<Json<BaseResponse<String>>> {
    ...
    pub async fn delete_sys_notice(rb: &RBatis, item: DeleteNoticeReq) -> AppResult<Json<BaseResponse<String>>> {
        Notice::delete_by_map(rb, value! {"id": &item.ids}).await.map(|_| ok_result())?
    }

    pub async fn update_sys_notice(rb: &RBatis, item: NoticeReq) -> AppResult<Json<BaseResponse<String>>> {
        let id = item.id;
        if item.id.is_none() {
    ...
    pub async fn update_sys_notice_status(rb: &RBatis, item: UpdateNoticeStatusReq) -> AppResult<Json<BaseResponse<String>>> {
        SysNoticeDao::update_status(rb, &item.ids, item.status).await.map(|_| ok_result())?
    }

    pub async fn query_sys_notice_detail(rb: &RBatis, item: QueryNoticeDetailReq) -> AppResult<Json<BaseResponse<NoticeResp>>> {
    ...
    pub async fn query_sys_notice_list(rb: &RBatis, item: QueryNoticeListReq) -> AppResult<Json<PageResponse<Vec<NoticeResp>>>> {
    ...
    pub async fn query_sys_notice_request(_rb: &RBatis, _item: QueryNoticeListReq) -> AppResult<Json<EmptyResponse>> {
