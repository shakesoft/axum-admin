...
impl SysDeptDao {
    ...
    pub async fn update_dept_status(rb: &RBatis, ancestors: &str, status: i8) -> Result<ExecResult, Error> {
        ...
        // 过滤空字符串
        let ids: Vec<i64> = ancestors.split(',').filter(|s| !s.is_empty()).map(|s| s.i64()).collect();
        ...
        let update_sql = format!("update sys_dept set status = ? , update_time = ? where id in ({})", placeholders);
