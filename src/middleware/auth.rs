...
pub async fn auth(State(state): State<Arc<AppState>>, mut req: Request, next: Next) -> Result<response::Response, StatusCode> {
    ...
    let ignore_paths = vec!["/system/user/login", "/system/user/register", "/system/user/logout"];
    ...
    match validate_and_get_user_info(&state.redis, jwt_token.id).await {
        Ok((user_id, permissions, roles, redis_token, is_admin)) => {
            ...
            let has_permission = |permissions: &[String], path: &str| -> bool { permissions.iter().any(|permission| permission == path) };
        ...
        Err(e) => match e {
            AppError::BusinessError(msg) if msg == "用户未登录" => Ok(AppError::JwtTokenError(msg.to_string()).into_response()),
...
async fn validate_and_get_user_info(redis_client: &Client, user_id: i64) -> Result<(i64, Vec<String>, Vec<String>, String, bool), AppError> {
    ...
    let vec_permissions: Vec<String> = vec_permissions.iter().map(|p| p.strip_prefix("/api").unwrap_or(p).to_string()).collect();
